%This script produces Gillespie simulations according to either given or randomly 
%generated paraemter sets for a given input network configuation that includes 
%multiple alleles (tested up to two alleles). Each allele is computationally distinguishable
%but functionally identical
%This function is specific for network configurations including negative regulation
%
%INPUT:
%
...
%                   constrained/asym not used implemented for this function
%rand_gen:          'yes' = generate a random seed for gillespie; 'no' use
%                   optional argument rand_set to provide a random seed for reproducibility
%rand_set:          optional numeric seed to RNG for gillepsie
%savefileID:        user-supplied unique tag for the experiment
%
%
%OUTPUT:
%
%S_outpar:           matrix of size 3*n_species*n_allele times maxgillespie, where
%                   the columns give the gene produt count and DNA state
%                   (on/off) per time

%%
function generateData_allele_diffRNA_neg_reg(nruns,n_species,n_alleles,gillespiesave,maxgillespie,gen,init,data,type,rand_gen,rand_set,savefileID)
...
if isequal(rand_gen, 'yes') == 1
    rand_seed = sum(clock*100);
else
    rand_seed = rand_set;
end

%load custom negative regulation networks

if isequal(type,'normal') == 1
    load_M_iso = sprintf('./../Data/M_iso_neg_reg%d',n_species);
    load(load_M_iso);
...
for istruc = 1:nstruc
    ...
    S = zeros(nruns,3*n_species*n_alleles);
    R = zeros(nruns,8*n_species*n_alleles);
    P = zeros(nruns,4*n_species*n_alleles);
    ...
    for iruns = 1:nruns
        ...
        clearvars -except gillespiesave savefileID nruns maxgillespie n_species init gen...
            ...
            iruns istruc S P R type n_alleles rand_seed rand_seed
        
        parmat = M_iso{istruc};
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        fileID = fopen('./../Data/gillespie_bursts.txt','w');
        trycount=0;
        while fileID < 0
            fileID
            pause(1)
        ...
        tok = make_param_mex_bursts_allele_diffRNA_neg_reg('../Data/gillespie_bursts.txt',...
            '../Data/gillespie_bursts',n_species,n_alleles,iruns, 1);
    ...
    for kmem = 1
        ...
        parfor jruns = 1:nruns
            ...
            par_spec = S(20000*kmem-20000+jruns,:);
            par_rates = R(20000*kmem-20000+jruns,:);
            par_prop = P(20000*kmem-20000+jruns,:);

            [times,savespecies] = gillespie_burstshistomex(0,par_spec,par_rates,par_prop,rand_seed,gillespiesave,maxgillespie);
