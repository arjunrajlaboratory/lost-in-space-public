...
function uniquetok = make_param_mex_bursts_allele_diffRNA(inputfile, outputfile, n_species, n_alleles, iruns, optimization_loop_count)
...
inlines = cell(1,4*n_species*n_alleles);
onlines = cell(1,1);
b_lines = cell(1,n_species*n_alleles);
k_nlines = cell(1, n_species*n_alleles);
nlines = cell(1,n_species*n_alleles);
initiallines = cell(1,n_species*n_alleles);
...
ratelines = cell(1,4*n_species*n_alleles);            %Rates
lines = cell(1,4*n_species*n_alleles);
ratenames = cell(1,4*n_species*n_alleles);
rate= zeros(1,4*n_species*n_alleles);

on_ratelines = cell(1,n_species*n_alleles);           %Difference in production arte due to on-state of burst
on_ratenames = cell(1,n_species*n_alleles);
on_rate = zeros(1,n_species*n_alleles);

b_ratelines = cell(1,n_species*n_alleles);            %Basal values
b_ratenames = cell(1,n_species*n_alleles);
b_rate = zeros(1,n_species*n_alleles);

k_ratelines = cell(1,n_species*n_alleles);            %Dissociation constant k (Hill function)
k_ratenames = cell(1,n_species*n_alleles);
k_rate = zeros(1,n_species*n_alleles);

n_ratelines = cell(1,n_species*n_alleles);            %Hill coefficient k (Hill function)
n_ratenames = cell(1,n_species*n_alleles);
n_rate = zeros(1,n_species*n_alleles);
...
%     foutc = fopen(['/storage/scratch/users/lea.schuh/RajNetworkModeling/' outputfile 'histomex.c'],'w');
    ...
    for i = 1:numrxns
        ...
        for currrxn = 1:numrxns
            ...
            if ismember(currrxn,1:(n_species*n_alleles)) %Production
                for ion = 1:2
                    if ion ==1
                        fprintf(foutc,'*%s*%s',on_ratenames{currrxn},uniquetok{2*currrxn+(n_species*n_alleles)});
                    else
                        fprintf(foutc,'+%s*%s',ratenames{currrxn},uniquetok{2*currrxn+(n_species*n_alleles)-1});
                    end
                end
            elseif ismember(currrxn,n_species*n_alleles+1:2*n_species*n_alleles) %Degradation
                fprintf(foutc,'*%s', uniquetok{currrxn-n_species*n_alleles});
            elseif ismember(currrxn,2*n_species*n_alleles+1:3*n_species*n_alleles) %Burst on
                 jnew = currrxn-(2*n_species*n_alleles);
                 countjparmat = 0;
                 for jparmat = 1:n_species
                     ...
                     elseif parmat(jparmat,jreduced) == -1 && countjparmat == 0 %Look for all species for which the production is dependent on abundance of j

                         fprintf(foutc,'*(-1*pow(%s,%s)/(pow(%s,%s)+pow(%s,%s))',string_for_pow,n_ratenames{jparmat},...
                             k_ratenames{jparmat},n_ratenames{jparmat},string_for_pow,n_ratenames{jparmat});
                         countjparmat = countjparmat +1;
                 ...
                 fprintf(foutc,'*%s',uniquetok{2*jnew+n_species*n_alleles-1});
                 fprintf(foutc,'+%s*%s',b_ratenames{jnew},uniquetok{2*jnew+n_species*n_alleles-1});
            elseif ismember(currrxn,3*n_species*n_alleles+1:4*n_species*n_alleles) %Burst off
                jnew = currrxn-(n_species*n_alleles*3);
                fprintf(foutc,'*%s',uniquetok{2*jnew+n_species*n_alleles});
...
[fid, message] = fopen([outputfile,'params.m'],'w');
trycount = 0;
while fid < 0
    fid
    pause(1)
...
for currrxn = 1:numrxns
    ...
    if ismember(currrxn,1:(n_species*n_alleles)) %Production
        for ion = 1:2
            if ion ==1
                fprintf(fid,'*%s*%s',on_ratenames{currrxn},uniquetok{2*currrxn+(n_species*n_alleles)});
            else
                fprintf(fid,'+%s*%s',ratenames{currrxn},uniquetok{2*currrxn+(n_species*n_alleles)-1});
            end
        end
    elseif ismember(currrxn,n_species*n_alleles+1:2*n_species*n_alleles) %Degradation
        fprintf(fid,'*%s', uniquetok{currrxn-n_species*n_alleles});
    elseif ismember(currrxn,2*n_species*n_alleles+1:3*n_species*n_alleles) %Burst on
        jnew = currrxn-(2*n_species*n_alleles);
        countjparmat = 0;
        for jparmat = 1:n_species
            ...
            if parmat(jparmat,jreduced) == 1 && countjparmat == 0 %look for all species for which the production is dependent on abundance of j
                ...
                countjparmat = countjparmat +1;
            elseif parmat(jparmat,jreduced) == -1 && countjparmat == 0 %look for all species for which the production is dependent on abundance of j
                fprintf(fid,'*(-1 * ((%s)^%s)/(%s^%s+(%s)^%s)',string_for_pow,n_ratenames{jparmat},...
                    k_ratenames{jparmat},n_ratenames{jparmat},string_for_pow,n_ratenames{jparmat});
                countjparmat = countjparmat +1;
            elseif parmat(jparmat,jreduced) == 1
                fprintf(fid,'+((%s)^%s)/(%s^%s+(%s)^%s)',string_for_pow,n_ratenames{jparmat},...
        ...
        fprintf(fid,'*%s',uniquetok{2*jnew+n_species*n_alleles-1});
        fprintf(fid,'+%s*%s',b_ratenames{jnew},uniquetok{2*jnew+n_species*n_alleles-1});
    elseif ismember(currrxn,3*n_species*n_alleles+1:4*n_species*n_alleles) %Burst off
        jnew = currrxn-(n_species*n_alleles*3);
        fprintf(fid,'*%s',uniquetok{2*jnew+n_species*n_alleles});      
    end
