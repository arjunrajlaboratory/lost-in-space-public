...
function uniquetok = make_param_mex_bursts_signal_allele(inputfile, outputfile, n_species, n_alleles, iruns, optimization_loop_count, signal_bool, HARDCODED_SIGNAL_ADD)

% First thing is to read in inputfile
[fid, message] = fopen(inputfile);
trycount=0;
while fid < 0
    fid
    pause(1)
...
[finc, message] = fopen('basicgillespie.c');
trycount=0;
while finc < 0
    finc
    pause(1)
...
% if the user wants to use an external signal, it is treated
% as an additional node with one allele
% therefore the total number of species = n_species*n_alleles + 1
if signal_bool
...
inlines = cell(1,4*total_species);
onlines = cell(1,1);
b_lines = cell(1,total_species);
k_nlines = cell(1, total_species);
nlines = cell(1,total_species);
initiallines = cell(1,total_species);
% at this stage, the network is the same as a monoallelic case
...
ratelines = cell(1,4*total_species);            %Rates
lines = cell(1,4*total_species);
ratenames = cell(1,4*total_species);
rate= zeros(1,4*total_species);

on_ratelines = cell(1,total_species);           %Difference in production arte due to on-state of burst
on_ratenames = cell(1,total_species);
on_rate = zeros(1,total_species);

b_ratelines = cell(1,total_species);            %Basal values
b_ratenames = cell(1,total_species);
b_rate = zeros(1,total_species);

k_ratelines = cell(1,total_species);            %Dissociation constant k (Hill function)
k_ratenames = cell(1,total_species);
k_rate = zeros(1,total_species);

n_ratelines = cell(1,total_species);            %Hill coefficient k (Hill function)
n_ratenames = cell(1,total_species);
n_rate = zeros(1,total_species);
...
if iruns == 1 && optimization_loop_count == 1
...
%     foutc = fopen(['/storage/scratch/users/lea.schuh/RajNetworkModeling/' outputfile 'histomex.c'],'w');
    ...
    for i = 1:numrxns
        ...
        for currrxn = 1:numrxns
            ...
            if ismember(currrxn,1:total_species) %Production
                for ion = 1:2
                    if ion ==1
                        fprintf(foutc,'*%s*%s',on_ratenames{currrxn},uniquetok{2*currrxn+total_species});
                    else
                        fprintf(foutc,'+%s*%s',ratenames{currrxn},uniquetok{2*currrxn+total_species-1});
                    end
                end
            elseif ismember(currrxn,total_species+1:2*total_species) %Degradation
                fprintf(foutc,'*%s', uniquetok{currrxn-total_species});
            elseif ismember(currrxn,2*total_species+1:3*total_species) %Burst on
                 jnew = currrxn-(2*total_species);
                 countjparmat = 0;
                 for jparmat = 1:n_species
                     if jnew > n_species
                        jreduced = mod(jnew,n_species);
                        if jreduced == 0
                            jreduced = n_species;
                        end
                     else
                     ...
                     for iallele = 1:n_alleles
                         if iallele == 1
                            string_for_pow = uniquetok{jparmat};
                         else
                             string_for_pow = strcat(string_for_pow, " + ", uniquetok{jparmat+ n_species*(iallele-1)});
                         end

                     end
                     if parmat(jparmat,jreduced) == 1 && countjparmat == 0 %Look for all species for which the production is dependent on abundance of j

                         fprintf(foutc,'*(pow(%s,%s)/(pow(%s,%s)+pow(%s,%s))',string_for_pow,n_ratenames{jparmat},...
                             k_ratenames{jparmat},n_ratenames{jparmat},string_for_pow,n_ratenames{jparmat});
                         countjparmat = countjparmat +1;
                     elseif parmat(jparmat,jreduced) == 1
                         fprintf(foutc,'+pow(%s,%s)/(pow(%s,%s)+pow(%s,%s))',string_for_pow,n_ratenames{jparmat},...
                             k_ratenames{jparmat},n_ratenames{jparmat},string_for_pow,n_ratenames{jparmat});
                         countjparmat = countjparmat +1;
                     end
                     if jparmat == n_species && countjparmat ~= 0
                         fprintf(foutc,')');
                     end
                 end
                 if countjparmat == 0
                 ...
                 fprintf(foutc,'*%s',uniquetok{2*jnew+total_species-1});
                 if jreduced == 1  && jnew ~= total_species
                     fprintf(foutc,'+%.1f*(pow(%s,%s)/(pow(%s,%s)+pow(%s,%s)))*%s',HARDCODED_SIGNAL_ADD,uniquetok{total_species},n_ratenames{jparmat},...
                             k_ratenames{jparmat},n_ratenames{jparmat},uniquetok{total_species},n_ratenames{jparmat},uniquetok{2*jnew+total_species-1});
                 end
                 fprintf(foutc,'+%s*%s',b_ratenames{jnew},uniquetok{2*jnew+total_species-1});
            elseif ismember(currrxn,3*total_species+1:4*total_species) %Burst off
                jnew = currrxn-(total_species*3);
                fprintf(foutc,'*%s',uniquetok{2*jnew+total_species});
...
for currrxn = 1:numrxns
    ...
    if ismember(currrxn,1:total_species) %Production
        for ion = 1:2
            if ion ==1
                fprintf(fid,'*%s*%s',on_ratenames{currrxn},uniquetok{2*currrxn+total_species});
            else
                fprintf(fid,'+%s*%s',ratenames{currrxn},uniquetok{2*currrxn+total_species-1});
            end
        end
    elseif ismember(currrxn,total_species+1:2*total_species) %Degradation
        fprintf(fid,'*%s', uniquetok{currrxn-total_species});
    elseif ismember(currrxn,2*total_species+1:3*total_species) %Burst on
        jnew = currrxn-(2*total_species);
        countjparmat = 0;
        for jparmat = 1:n_species
            ...
            if jreduced == 1
                string_for_pow = strcat(string_for_pow, " + ", uniquetok{total_species});
            end
            if parmat(jparmat,jreduced) == 1 && countjparmat == 0 %look for all species for which the production is dependent on abundance of j
                fprintf(fid,'*(((%s)^%s)/(%s^%s+(%s)^%s)',string_for_pow,n_ratenames{jparmat},...
                    k_ratenames{jparmat},n_ratenames{jparmat},string_for_pow,n_ratenames{jparmat});
                countjparmat = countjparmat +1;
            elseif parmat(jparmat,jreduced) == 1
                fprintf(fid,'+((%s)^%s)/(%s^%s+(%s)^%s)',string_for_pow,n_ratenames{jparmat},...
        ...
        fprintf(fid,'*%s',uniquetok{2*jnew+total_species-1});
        fprintf(fid,'+%s*%s',b_ratenames{jnew},uniquetok{2*jnew+total_species-1});
    elseif ismember(currrxn,3*total_species+1:4*total_species) %Burst off
        jnew = currrxn-(total_species*3);
        fprintf(fid,'*%s',uniquetok{2*jnew+total_species});      
    end
