%Analyze the 2-5 node, all subnets, vary off add k 700 parameter ensemble
% with a shorter run length for instances where a smaller individual file
% size is useful for plotting
...
for irep = 1:5
    ...
    writematrix(export_bursts, strcat(save_directory, sprintf('bursts-rep%d.csv', irep)));
writematrix(export_OR, strcat(save_directory, sprintf('OR-rep%d.csv', irep)));
end
